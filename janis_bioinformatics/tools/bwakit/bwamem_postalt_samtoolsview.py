...
from janis_bioinformatics.data_types import FastaWithDict, FastqGzPair, Bam, Bed, FastaAlt
...
class BwaMem_PostAlt_SamToolsView(BioinformaticsTool):
    def tool(self) -> str:
        return "BwaMemPostAltSamtoolsView"
    ...
    def container(self):
        return "biocontainers/bwakit:v0.7.15_cv1"
    ...
    def arguments(self):
        return [
            ...
            ToolArgument("k8", position=6, shell_quote=False),
            ToolArgument("/opt/conda/bin/bwa-postalt.js", position=7, shell_quote=False),
            ToolArgument(InputSelector("referenceAlt"), position=8, shell_quote=False),
            # Samtools View command
            ToolArgument("|", position=9, shell_quote=False),
            ToolArgument("samtools", position=10, shell_quote=False),
            ToolArgument("veiw", position=11, shell_quote=False),
            ToolArgument(CpuSelector(), position=12, shell_quote=False),
            ToolArgument(
                "-h",
                position=8,
                shell_quote=False,
    ...
    def friendly_name(self) -> str:
        return "Bwa mem + alt aware post-processing + Samtools View"

    def bind_metadata(self):
        return ToolMetadata(
            contributors=["Miriam M Yeung"],
            dateCreated=datetime(2021, 7, 14),
            dateUpdated=datetime(2021, 7, 14),
        )
...
if __name__ == "__main__":
    print(BwaMem_PostAlt_SamToolsView().translate("wdl"))
